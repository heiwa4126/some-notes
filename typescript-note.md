...
- [TypeScript には「型空間(type space)」「値空間(value space)」「名前空間(namespace space)」があり、同じ識別子でも別空間なら衝突しない](#typescript-には型空間type-space値空間value-space名前空間namespace-spaceがあり同じ識別子でも別空間なら衝突しない)
...
## TypeScript には「型空間(type space)」「値空間(value space)」「名前空間(namespace space)」があり、同じ識別子でも別空間なら衝突しない
...
ただし複数空間にまたがってるやつ(例: class) があるので注意

| 宣言      | Type | Value | Namespace |
| --------- | ---- | ----- | --------- |
| type      | ✓    |       |           |
| interface | ✓    |       |           |
| const/let |      | ✓     |           |
...
上の表で "namespaceがvalueとnamespaceの両方にある" 話が
"declaration merging" のよい例で
...
- class User が value/type を提供
- namespace User が namespace/value を追加
