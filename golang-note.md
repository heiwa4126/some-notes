...
- [どのバージョンのgoでビルドされたかを知る](#どのバージョンのgoでビルドされたかを知る)
- [Go アプリをDocker イメージにするときに](#go-アプリをdocker-イメージにするときに)
  - [1. `-ldflags="-w -s"` はするべき?(推奨:YES)](#1--ldflags-w--s-はするべき推奨yes)
  - [2. `upx` はするべき?(推奨:NO)](#2-upx-はするべき推奨no)
  - [3. Googleではどうしてる? (osv-scanner などの例)](#3-googleではどうしてる-osv-scanner-などの例)
    - [Googleの標準的なアプローチ](#googleの標準的なアプローチ)
  - [まとめ:ベストプラクティス構成](#まとめベストプラクティス構成)
  - [比較表](#比較表)
  - [補足: ここ人間](#補足-ここ人間)
- [では次の比較として](#では次の比較として)
...
## どのバージョンのgoでビルドされたかを知る

よくある govulncheck の例 (Goバージョンのかわりめに起きやすい)

```sh
go version "$(command -v govulncheck)"

# 依存・ビルド設定まで欲しいなら
go version -m "$(command -v govulncheck)"
```

## Go アプリをDocker イメージにするときに
...
もちろんケース・バイ・ケースでしょうけど、
こういうのは Google がよくやってるパターンなので Gemini に聞いてみた。

ちょっといい答えだと思うので、コピペします。

---

結論から言うと、モダンなコンテナ開発(特にGoogle流のやり方)においては、**`-ldflags="-w -s"` は推奨されますが、`upx` は避けるのが一般的**です。

それぞれの理由を、サイズ・セキュリティ・運用性の観点から整理します。

### 1. `-ldflags="-w -s"` はするべき?(推奨:YES)

Goのコンパイル時に `-s` (シンボルテーブルの削除) と `-w` (DWARFデバッグ情報の削除) を付与するのは、コンテナイメージを軽量化する際の「鉄板」の手法です。

**考察**

- **サイズ:** バイナリサイズを20%〜30%ほど削減できます。
- **セキュリティ:** バイナリを解析する際の手がかり(関数名や行番号など)が消えるため、リバースエンジニアリングの難易度がわずかに上がります。
- **運用の注意点:** パニック時のスタックトレースに詳細な情報(ファイル名や行番号)が出なくなります。ただし、Sentryなどのエラー監視ツールを適切に使っていれば、デバッグ情報の欠如は運用でカバー可能です。

### 2. `upx` はするべき?(推奨:NO)

バイナリを圧縮する `upx` は、かつてのリソースが乏しい時代には重宝されましたが、現在のクラウドネイティブな環境ではデメリットが目立ちます。

**考察**

- **サイズ:** 劇的に小さくなります。
- **セキュリティ (最重要):** `osv-scanner` や `Trivy` などの脆弱性スキャナは、バイナリ内の静的解析を行いますが、**`upx` で圧縮されていると中身が解析できず、脆弱性を見逃すリスク**があります。また、実行時にメモリ上で展開される挙動がマルウェアに似ているため、セキュリティソフトに検知されやすくなります。
- **パフォーマンス:** 実行のたびにメモリ上で解凍プロセスが走るため、起動時間(Startup Latency)が延び、メモリ消費量も増えます。

### 3. Googleではどうしてる? (osv-scanner などの例)

Googleが公開している Go 製ツールの多く(`osv-scanner` や `ko`、`kaniko` など)のビルドプロセスを見ると、彼らの哲学が見えてきます。

#### Googleの標準的なアプローチ

Googleはバイナリのサイズを削ることよりも、**「透明性(Transparency)」と「再現性」** を重視します。

- **Distroless Image の利用:** Googleはバイナリを小さくする代わりに、実行環境であるOSイメージを極限まで削った **Distroless** を提唱しています。OS側の脆弱性を排除する方が、バイナリを数MB削るよりセキュリティ効果が高いからです。
- **osv-scanner の例:** `osv-scanner` 自体のリリースバイナリも、過度な圧縮(upx)はされていません。これは、サプライチェーン・セキュリティにおいて「何が含まれているか」を外部から検証可能(Scannable)にしておく必要があるためです。
- **Goのビルドオプション:** Googleが主導するプロジェクトでは、`-ldflags="-w -s"` はよく使われますが、それ以上に **`CGO_ENABLED=0` による完全静的リンク** を徹底し、実行環境に依存しないポータビリティを確保することを優先します。

### まとめ:ベストプラクティス構成

DockerでGoを実行するなら、以下の構成が「最もGoogle流に近い、モダンで安全な構成」と言えます。

1. **Build:** `CGO_ENABLED=0 go build -ldflags="-s -w" -o app`
2. **Compress:** `upx` は使わない(スキャナの透過性を維持するため)。
3. **Base Image:** `gcr.io/distroless/static-debian12`(または `scratch`)を使用。

### 比較表

| 項目             | `-ldflags="-s -w"`   | `upx`                    |
| ---------------- | -------------------- | ------------------------ |
| **サイズ削減**   | ◯ (そこそこ削れる)   | ◎ (劇的に削れる)         |
| **起動速度**     | 影響なし             | △ (解凍により遅くなる)   |
| **スキャナ対応** | ◎ (問題なく検知可能) | × (脆弱性検知を阻害する) |
| **Googleの傾向** | 一般的に使用される   | ほぼ使われない           |

### 補足: ここ人間

[osv-scanner/Dockerfile at main · google/osv-scanner](https://github.com/google/osv-scanner/blob/main/Dockerfile)を見ると
マルチステージビルドでどっちもalpine.

これは「gitが要るから」という理由がある。

## では次の比較として

- scratch + zoneinfo.zip
- Distroless/static

どっちがいい? これも gemini にきいてみた。
