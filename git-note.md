...
- [git blame](#git-blame)
- [ブランチをそっくり別のブランチで置き換える(ただし履歴は残す)](#ブランチをそっくり別のブランチで置き換えるただし履歴は残す)
...
# 誤って削除してしまったファイルを復活させる
rm hello.c
git restore hello.c

# 現在のディレクトリ以下のすべての変更を取り消す
git restore .

# ステージングの取り消し
# ファイル自体の変更はワークスペースに残したまま、`git add` の効果だけを取り消します
git restore --staged hello.c

# 両方の変更を完全にリセットする
# `--staged` と `--worktree`(または `-S -W`)を同時に指定することで、
# ステージングエリアと作業ツリーの両方を一度に特定のコミットの状態まで巻き戻せます。
it restore --source=HEAD --staged --worktree hello.c


# 2つ前のコミットから特定のファイルを復元する:**
# 過去の特定のコミットや別ブランチから、ファイルの内容を引っ張ってきたいとき
# `--source`(または `-s`)オプションを使うことで、現在の状態ではなく、過去のコミットや別のブランチ、タグからファイルの状態を復元できます。
git restore --source master~2 Makefile

# 対話パッチモード
# 変更の一部だけを**対話形式**で元に戻したいとき
# ファイル全体の変更を戻すのではなく、修正したコードの一部(ハンク単位)だけを元に戻したい場合に使います
git restore -p
```
...
2. **HEAD 相対参照を使う方法**
   ...
   `HEAD`はカレントコミットを指しています。`HEAD`からの相対位置を指定することで、特定のコミットに移動できます。
...
4. **タグを使う方法**
   ...
   移動先のコミットがデタッチド HEAD の状態になるので、作業ブランチを作成する場合は以下のようにします。
...
## Git リポジトリの履歴を完全にクリーンアップして、不要なオブジェクトを削除する

普段は `git gc` (自動で実行される) で充分。

Git リポジトリの履歴を完全にクリーンアップして、
不要なオブジェクトを削除するには
...
2. `git reflog expire --expire=now --all`
   - **reflog** は、ブランチや HEAD の過去の移動履歴を記録しているログです。
   - `--expire=now --all` は、**すべての reflog を即時期限切れにする**という意味。
   - 意図: 過去のコミットへの参照を消し、復元できないようにする
3. `git gc --prune=now --aggressive`
   - `git gc` は **ガーベジコレクション**で、不要なオブジェクトを削除し、リポジトリを最適化します。
   - `--prune=now` は、**参照されていないオブジェクトを即時削除**。
   - `--aggressive` は、圧縮を強化してリポジトリサイズを最小化。
   - 意図: 履歴削除後の不要データを完全に消し、リポジトリを軽量化

⚠ **注意点**

- 一度実行すると、過去の履歴はほぼ完全に消えます。**元に戻せません**
- 公開リポジトリでやると、他のクローンとの整合性が崩れるので危険

## いまの状態から「1個前」を新しいブランチにしてチェックアウト
...
便利な機能なんだけど、なんで "blame(非難する)" ?

## ブランチをそっくり別のブランチで置き換える(ただし履歴は残す)

ブランチBの内容をすべて別のブランチAの内容で置き換える場合(Bの履歴は残す)、
「ours 戦略のマージ」を使う
...
# 一時ブランチに、ブランチBを「ours」戦略でマージ
git merge -s ours B
## 履歴はAとBが混ざるが、中身は100% A

# ブランチBに切り替え
git checkout B

# 一時ブランチの内容を、ブランチBに流し込み
git merge tmp
