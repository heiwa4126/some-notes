...
> Unity は、ランタイムの前(ユーザーがエクスペリエンスに触れる前)にベイクドライティングを事前計算します。つまり、Unity が実行時に行う必要があるのは、ライティングデータをシーンに適用することだけです。
> 実行時にベイクされたライトのプロパティを変更することはできず、動的な GameObject(シーン内を歩くキャラクターなど)には反応しません。

> ライティングを計算するプロセスでは、マテリアルでテクスチャをマッピングするために使用される UV 座標のように、シーン全体の UV 座標を生成します。このテクスチャマップはライトマップと呼ばれ、その作成プロセスはライトマッピングまたはベイクと呼ばれます。ベイクドライティングはより効率的ですが、よりリアルでユーザーを魅了するライティングを実装するためにも使用できます。

[2.What is baked lighting?](https://learn.unity.com/tutorial/bake-a-lightmap-for-your-scene?uv=2022.3&pathwayId=61a65568edbc2a00206076dd&missionId=61a106efedbc2a5554f0dcba#64d0b792edbc2a1afc67e41a)
