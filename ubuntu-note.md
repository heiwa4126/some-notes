...
- [カーネルをn世代分残す](#カーネルをn世代分残す)
- [ubuntu の timesyncd](#ubuntu-の-timesyncd)
- [パッケージの再設定のやりなおし](#パッケージの再設定のやりなおし)
- [logwatch の設定](#logwatch-の設定)
- [cron-apt](#cron-apt)
- [unattended-upgrades](#unattended-upgrades)
  - [unattended-upgrades はいつ実行されるか?](#unattended-upgrades-はいつ実行されるか)
  - [unattended-upgrades があれば cron-apt は消してもいい?](#unattended-upgrades-があれば-cron-apt-は消してもいい)
  - [`/usr/lib/apt/apt.systemd.daily` のざっくり説明](#usrlibaptaptsystemddaily-のざっくり説明)
- [Ubuntu の http proxy 設定](#ubuntu-の-http-proxy-設定)
...
`/etc/apt/apt.conf.d/01autoremove` に

```text
APT::AutoRemove::Keep-Kernels "3";
```

みたいに書いて制御する。

以下はさくらのVPNにあった設定なんだけど、これだと絶対に消えないで、手動削除になる
...
journalctl -u systemd-timesyncd -r
...
Logwatch は設定を階層的に読み込みむ

1. /usr/share/logwatch/default.conf/logwatch.conf  
   (パッケージ付属のデフォルト。こちらは編集しない)
2. /etc/logwatch/conf/logwatch.conf  
   (管理者override)

最終的な設定結果を表示する機能はないので

```sh
logwatch --debug high --output stdout
```
...
みたいな設定になっている。これは
「自動インストールはしないが、更新パッケージは事前ダウンロードしておく」
設定。

## unattended-upgrades

これ設定がややこしいし、何をするパッケージか、何がトリガかよくわからない。とりあえずデフォルトでは
...
で、これは "セキュリティ修正のみ自動適用" の設定。

### unattended-upgrades はいつ実行されるか?

実行トリガは systemd timer(apt-daily / apt-daily-upgrade)

unattended-upgrades.service 自体は本体じゃなく「シャットダウン待機用」。

```console
$ systemctl list-timers | grep -Fi apt-daily
Wed 2026-06-10 21:49:20 JST          10h Wed 2026-06-10 06:38:44 JST 4h 32min ago apt-daily.timer                apt-daily.service
Thu 2026-06-11 06:21:03 JST          19h Wed 2026-06-10 06:24:44 JST 4h 46min ago apt-daily-upgrade.timer        apt-daily-upgrade.service
```

- apt-daily.timer - `apt update` 担当
- apt-daily-upgrade.timer - unattended-upgrades 実行
...
の `ExecStart=` にある `/usr/lib/apt/apt.systemd.daily` が
unattended-upgrades を実行するしくみ。

なんでこんなにややこしいのか。歴史というもの

### unattended-upgrades があれば cron-apt は消してもいい?

unattended-upgrades で
"セキュリティ修正のみ自動適用"
なら、cron-apt で アップデート分をダウンロードしておくことには意味がある。

### `/usr/lib/apt/apt.systemd.daily` のざっくり説明

実際に見ると目がつぶれそうなスクリプトなのでChatGPTにまとめてもらった。

簡潔に言うと、
「APTの定期メンテナンスをまとめて実行するドライバースクリプト」
です。

apt-daily.service や apt-daily-upgrade.service から呼ばれ、
...
一言で言えば「APTの定期処理の司令塔」です。

unattended-upgrade 本体ではなく、それを含む複数の処理を振り分けるラッパーです。

## Ubuntu の http proxy 設定
