...
## `npm i`, `npm ci`, `npm up` の違い

以下に、`npm i`(または `npm install`)、`npm ci`、および`npm up`(または `npm update`)の違いを表形式で示します。

| 項目                          | `npm i` (`npm install`)                                           | `npm ci`                                                                    | `npm up` (`npm update`)                          |
| ----------------------------- | ----------------------------------------------------------------- | --------------------------------------------------------------------------- | ------------------------------------------------ |
| 主な用途                      | 依存関係のインストールと更新                                      | 依存関係のクリーンインストール                                              | 依存関係の更新                                   |
| インストール方法              | `package.json`を基にインストールし、`package-lock.json`を更新する | `package-lock.json`を基にインストールし、`package.json`と一致するか確認する | `package.json`に基づいて既存の依存関係を更新する |
| `package-lock.json`の取り扱い | 更新する場合がある                                                | 常に存在し、一切変更しない                                                  | `package.json`に基づいて更新する                 |
| 新しい依存関係の追加          | 可能                                                              | 不可能                                                                      | 不可能                                           |
| インストールの速度            | 遅い(依存関係の解決とダウンロード)                                | 速い(すでに確定された依存関係のインストール)                                | 遅い(依存関係の解決とダウンロード)               |
| 既存の`node_modules`の処理    | そのまま使用するか、必要に応じて更新する                          | 削除してから再インストール                                                  | 更新された依存関係のみインストールする           |
| 使用場面                      | 開発中の依存関係の管理や追加                                      | CI/CD パイプラインやクリーンインストールが必要な場合                        | 依存関係のバージョンを最新に保つ必要がある場合   |
...
## npm publish の tag について

- おなじタグはつけられない。あとから publish したものに奪われる
...
```sh
mkdir -p ~/.config/npm
touch ~/.config/npm/npmrc
nano ~/.config/npm/npmrc
```
