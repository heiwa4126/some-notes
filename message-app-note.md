...
au は「EZweb メール」、
ソフトバンクは「S!メール」
が MMS。ただしそれぞれ独自の実装で、本物の MMS 規格と互換が無い。
...
## プラスメッセージが使えない日本の通信事業者は

楽天だけらしい(サブブランド、MVNO も含めて)。

[＋メッセージ（プラスメッセージ）の利用はできますか？ | お客様サポート | 楽天モバイル](https://network.mobile.rakuten.co.jp/faq/detail/00001137/)

SMS は楽天モバイル、ドコモ、au、ソフトバンクのすべての間で送受信可能。

あと、
いまのところプラスメッセージは日本ローカル。
