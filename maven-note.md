...
別々の実行ファイルに ID を与えることで、継承やプロファイルの適用時にゴールの設定をマージするか、追加の実行ファイルにするかを制御できます。
...
バージョンは配布しているのであれば、数字とドット(1.0, 1.1, 1.0.1, ...)を使った典型的なバージョンを選べばよいでしょう。
