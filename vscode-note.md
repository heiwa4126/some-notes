...
## editor.autoSurround 機能

`(` を押すと、選択範囲が()でくくられる機能。
クォート全般、カッコ全般、markdownの `*`, `**`, <code>`</code> で使える
...
他にも「カーソル位置の単語と同じもの」系があるんだけど、なんか思わぬところが選択されて辛い。

## キーボードショートカットに何が割り当てられているか

Emacsの C-h k (describe-key) みたいなやつ

1. Ctrl + K → Ctrl + S で "キーボードショートカット" を表示する
2. "入力してキーバインド内を検索" のところにあるキーボードアイコンをクリック
3. キーを押す
