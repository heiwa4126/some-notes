...
いまのところ(2026-05-25) VSCode の拡張で Mermaid を含む Marp を PDF に出すのは難しい。
mermaid.js を PDFに入れるのはセキュリティダイアログがうるさいのでやりたくない。

現実的な対策としては:

- Mermaid ブロックを SVGファイルに変換して、それへのリンクにした Markdown ファイルを作る
- または SVG は Markdown に埋め込めるのだから、埋め込んだ Markdown に変換する

こういうツールはすでに実在する。
で、その Markdownをレンダリングする感じ。
...
本当はそんな中間段階は不要で、いきなりPDFが出ればいいんですけど、そーいうのは無い。
shell script かませばできる。

## Marp 代替

[なぜ Slidev か \| Slidev](https://ja.sli.dev/guide/why#%E6%AF%94%E8%BC%83)
